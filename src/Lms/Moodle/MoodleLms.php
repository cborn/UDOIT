...
class MoodleLms implements LmsInterface {
    ...
    public function getLtiAuthUrl($globalParams)
    {
        ...
        return "{$baseUrl}/moodle/lti/authenticate?{$queryStr}";
    }
    ...
    public function getKeysetUrl()
    {
        ...
        return $session->get('iss') . '/moodle/.well-known/jwks';
    }
    ...
    public function postContentItem(ContentItem $contentItem)
    {
        ...
        if ('file' === $contentItem->getContentType()) {
            ...
            $lmsResponse = $this->moodleApi->apiFilePut($url, $filepath);
        }
        else {
            $lmsResponse = $this->moodleApi->apiPut($url, ['body' => \json_encode($options)]);
        }
    ...
    {
        return "https://{$this->baseUrl}/moodle/home/{$course->getLmsCourseId()}";
    }
    ...
    public function updateFileItem(Course $course, $file)
    {
        ...
        if (!$fileItem) {
            ...
            $lmsUrl = "https://{$domainName}/moodle/le/content/{$course->getLmsCourseId()}/viewContent/{$file['id']}/View";
    ...
    protected function setMoodleApi(User $user)
    {
        $apiDomain = $this->getApiDomain($user);
        $apiToken = $this->getApiToken($user);
        
        $this->moodleApi = new MoodleApi($apiDomain, $apiToken);
    }
    ...
    protected function createLmsPostOptions(ContentItem $contentItem)
    {
        ...
        $url = $this->getContentItemApiUrl($contentItem);
        $content = $this->moodleApi->apiGet($url)->getContent();
    ...
    protected function getCourseOverview(Course $course)
    {
        ...
        $baseUrl = "https://{$lmsDomain}/moodle/home/{$course->getLmsCourseId()}";
    ...
    protected function getItemsFromModule($module, Course $course)
    {
        ...
        $domainName = $course->getInstitution()->getLmsDomain();
        $contentUrl = "https://{$domainName}/moodle/le/content/{$course->getLmsCourseId()}/Home";
        ...
        foreach ($module['Topics'] as $topic) {
            $topicUrl = "https://{$domainName}/moodle/le/content/{$course->getLmsCourseId()}/viewContent/{$topic['TopicId']}/View";
